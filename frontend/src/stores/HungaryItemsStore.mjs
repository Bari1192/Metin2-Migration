...
export const useHungaryStore = defineStore("items", () => {
  ...
  async function getItem(identifier) {
    ...
    loading.value = true;
    try {
      const endpoint = typeof identifier === 'number' || /^\d+$/.test(identifier) 
        ? `/items/${identifier}` : `/items/by-name/${identifier}`;
  ...
  async function updateItem(identifier, itemData) {
    ...
    try {
      ...
      const resp = await http.put(`/items/${identifier}`, itemData, { headers: authHeaders });
